...
const ListView: React.FC = () => {
	...
	const [selectedSection, setSelectedSection] = useState<'movie' | 'book' | 'album'>(
		mediaTypeFromUrl
	);
		...
		<div className="page-container">
			...
			{canEdit && (
				<SearchBar
					...
					mediaType={selectedSection}
					setSelectedMediaType={setSelectedSection}
					selectedMediaType={selectedSection}
				/>
			)}
				...
				<div className="tabs">
					{movies.length > 0 && (
						<button
							...
							className={`tab-button ${selectedSection === 'movie' ? 'active' : ''}`}
						>
					...
					{books.length > 0 && (
						<button
							...
							className={`tab-button ${selectedSection === 'book' ? 'active' : ''}`}
						>
					...
					{albums.length > 0 && (
						<button
							...
							className={`tab-button ${selectedSection === 'album' ? 'active' : ''}`}
						>
