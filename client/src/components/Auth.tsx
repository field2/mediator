...
const Auth: React.FC = () => {
  ...
  const [view, setView] = useState<AuthView>(() => searchParams.get('mode') !== 'register' ? 'login' : 'register');
    ...
    <div style={{ maxWidth: '400px', margin: '50px auto', padding: '20px' }}>
      ...
      {view === 'login' && (
        <p style={{ marginTop: '15px', textAlign: 'center' }}>
          <a onClick={() => { setView('forgot'); setError(''); }} className="auth-toggle-link">
            Forgot password?
          </a>
        </p>
      )}
      ...
      {(view === 'forgot' || view === 'reset') && (
        <p style={{ marginTop: '20px', textAlign: 'center' }}>
          <a onClick={() => { setView('login'); setResetLinkSent(false); }} className="auth-toggle-link">
            Back to login
          </a>
        </p>
      )}
